...
export default function SellTableUpdateForm(props) {
  const {
    id: idProp,
    sellTable,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = sellTableRecord
      ? { ...initialValues, ...sellTableRecord }
      : initialValues;
    ...
    setImageUrl2(cleanValues.imageUrl2);
    setErrors({});
  };
  const [sellTableRecord, setSellTableRecord] = React.useState(sellTable);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(SellTable, idProp)
        : sellTable;
      setSellTableRecord(record);
    };
    queryData();
  }, [idProp, sellTable]);
  React.useEffect(resetStateValues, [sellTableRecord]);
  const validations = {
    goodsName: [],
    comment: [],
    PickupPlace: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            SellTable.copyOf(sellTableRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "SellTableUpdateForm")}
      {...rest}
    >
      <TextField
        label="Goods name"
