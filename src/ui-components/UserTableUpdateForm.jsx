...
export default function UserTableUpdateForm(props) {
  const {
    id: idProp,
    userTable,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = userTableRecord
      ? { ...initialValues, ...userTableRecord }
      : initialValues;
  ...
  const [userTableRecord, setUserTableRecord] = React.useState(userTable);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(UserTable, idProp)
        : userTable;
      setUserTableRecord(record);
    };
    queryData();
  }, [idProp, userTable]);
  React.useEffect(resetStateValues, [userTableRecord]);
  const validations = {
    userName: [],
    iconUrl: [],
    currentAddress: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            UserTable.copyOf(userTableRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "UserTableUpdateForm")}
      {...rest}
    >
      <TextField
        label="User name"
